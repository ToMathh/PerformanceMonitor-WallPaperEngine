"""
Run ONCE before building:  python make_icon_beta.py
Generates icon.ico (BLUE theme) for Beta version.
"""
import os, sys
...
def make():
    ...
    for s in sizes:
        ...
        d.rounded_rectangle([lw, lw, s-lw-1, s-lw-1], radius=max(2, s//8),
                             fill=(14, 14, 14, 255), outline=(0, 120, 215, 255), width=lw)
        # ears (two triangles) - BLUE
        ew = max(2, s // 5)
        eh = max(3, s // 3)
    ...
    print(f"[OK] icon.ico (BLUE) created at: {out}")
