"""
Run ONCE before building:  python make_icon_minimal.py
Generates icon.ico (GREEN theme) for UltraLowPerf version.
"""
import os, sys
...
def make():
    ...
    for s in sizes:
        ...
        d.rounded_rectangle([lw, lw, s-lw-1, s-lw-1], radius=max(2, s//8),
                             fill=(14, 14, 14, 255), outline=(0, 180, 80, 255), width=lw)
        # ears (two triangles) - GREEN
        ew = max(2, s // 5)
        eh = max(3, s // 3)
    ...
    print(f"[OK] icon.ico (GREEN) created at: {out}")
